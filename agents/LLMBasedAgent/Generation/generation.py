...
def text_based(recieved_offer_history, sent_offer_history, time, argument, model_type):
    ...
    if provider == "openai":
        response = Openai.text_based(model, prompt)
    elif provider == "claude":
        response = Claude.text_based(model, prompt)
    elif provider == "google":
        response = Google.text_based(model, prompt)
    else:
